...
function getFileExtension(filename) {
  ...
  return filename.slice(lastDotIndex).toLowerCase();  // 取副檔名並轉小寫 
}
...
function handleUpload(req, res, config) {
  ...
  form.on('error', (err) => {
    console.error('背景串流錯誤:', err.message);

    if (res.headersSent) return;
    if (err.message.includes('maxFileSizeExceeded')) {
      res.writeHead(413, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ error: '檔案太大' }));
    }
  });
